"""Weekly orchestration pipeline for full model retraining."""
...
def run_weekly_pipeline(
    ...
    runtime_baseline_dir = baseline_dir or (runtime_data_dir / "model_baselines")
    for path in [runtime_data_dir, runtime_artifact_dir, runtime_report_dir, runtime_log_dir, runtime_baseline_dir]:
        path.mkdir(parents=True, exist_ok=True)

    monitor = PipelineMonitor(log_dir=runtime_log_dir)
    monitor.start_run("weekly_pipeline")
    try:
        ...
        dq_report_path = write_data_quality_report(dq_reports, runtime_report_dir / "weekly_data_quality.json")
        ...
        demand_result = DemandForecastModel(artifact_dir=runtime_artifact_dir).train(pipeline_tables["transactions"])
        segmentation_result = CustomerSegmentationModel(artifact_dir=runtime_artifact_dir).train(
        ...
        model_monitor = ModelMonitor(baseline_dir=runtime_baseline_dir)
        if "product_features" in feature_outputs:
            model_monitor.save_baseline(feature_outputs["product_features"].reset_index(drop=True), "demand_forecast")
        if "customer_features" in feature_outputs:
            model_monitor.save_baseline(feature_outputs["customer_features"], "churn_prediction")
        monitor.log_step("save_baselines", rows_processed=len(feature_outputs))
        ...
        summary = {
            ...
            "model_outputs": {
                "demand_forecast_rows": len(demand_result["forecasts"]),
                "customer_segments_rows": len(segmentation_result["segments"]),
